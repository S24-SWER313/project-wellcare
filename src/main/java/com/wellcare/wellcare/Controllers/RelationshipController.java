...
public class RelationshipController {
         ...
         * relationship status = 1 => friend request is accepted, user1 and user2 are friend
         * relationship status = 2 => friend request is canceled, only if status = 0
         * 1. addFriend: user1 sends friend request to user2, so relationship status = 0 (pending request)
         * 2. cancelFriendshipRequest: user1 or user2 can cancel the friend request, so the status will equal 2, row won't be deleted from DB => only used if status = 0
         * 3. getFriendRequests: user1 or user2 can see their friend requests, where relationship status = 0
         * 4. acceptFriend: if the status = 0, user1 or user2 can accept the friend request came to them, so the status will become 1 (request accepted)
         * 5. removeFriend: if status = 1, user1 or user2 can unfriend the users from their friends list, so the unfriended user will be removed from the DB => only used if status = 1
         */
