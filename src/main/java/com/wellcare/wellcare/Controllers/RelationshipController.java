...
public class RelationshipController {
    ...
    public ResponseEntity<EntityModel<MessageResponse>> addFriend(HttpServletRequest request,
        ...
        try {
            ...
            Relationship relationshipFromDb = relationshipRepository
                    .findRelationshipByUserOneUsernameAndUserTwoUsername(loggedInUser.getUsername(), friendCandidateUser.getUsername());
            ...
            if (relationshipFromDb != null) {
                if (relationshipFromDb.getStatus() == 1) {
                    return ResponseEntity.badRequest()
                        ...
                        return ResponseEntity.ok(EntityModel.of(new MessageResponse("Friend request cancelled")));
                    ...
                    relationshipRepository.save(relationshipFromDb);
                    return ResponseEntity.ok(EntityModel.of(new MessageResponse("Friend request sent successfully")));
    ...
    public ResponseEntity<EntityModel<MessageResponse>> cancelFriendshipRequest(HttpServletRequest request,
        ...
        try {
            ...
            if (friendUsername == null) {
                return ResponseEntity.badRequest()
                        .body(EntityModel.of(new MessageResponse("Friend user name is required")));
            }

            // Check if a friend request exists between the two users
            Relationship relationship = relationshipRepository
                    .findRelationshipByUserOneUsernameAndUserTwoUsername(loggedInUser.getUsername(), friendUsername);

            // If no relationship is found, check the other way around
            if (relationship == null) {
                relationship = relationshipRepository
                        .findRelationshipByUserOneUsernameAndUserTwoUsername(friendUsername, loggedInUser.getUsername());
            }
            ...
            entityModel.add(linkTo(methodOn(RelationshipController.class).addFriend(request, friendUsername))
                    .withRel("addFriend"));
    ...
    public ResponseEntity<EntityModel<MessageResponse>> acceptFriend(HttpServletRequest request,
        ...
        try {
            ...
            boolean isFriendshipAccepted = changeStatusAndSave(loggedInUser.getUsername(), friendUsername, 0, 1);
    ...
    public ResponseEntity<MessageResponse> getFriendRequestStatus(HttpServletRequest request,
        ...
        try {
            ...
            if (relationship != null) {
                if (relationship.getStatus() == 0) {
                    if (relationship.getActionUser().getUsername().equals(friendUsername)) {
                        return ResponseEntity.ok().body(new MessageResponse("Friend request received"));
                    } else {
                        return ResponseEntity.ok().body(new MessageResponse("Friend request already sent"));
                    }
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new MessageResponse("Error checking friend request status: " + e.getMessage()));
        }
    }
    
    private boolean changeStatusAndSave(String loggedInUsername, String friendusername, int fromStatus, int toStatus)
            throws Exception {
        Relationship relationship = relationshipRepository
    ...
    public ResponseEntity<?> getUserFriendsCount(@PathVariable String username) {
        try {
                ...
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                        .body(new MessageResponse("Error fetching user friends count: " + e.getMessage()));
            }
    }
}
